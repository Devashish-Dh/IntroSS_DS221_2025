...
inline void flush_cache() {
    // 72 MB buffer (2× the 36 MB L3 size) to be safe
    static std::vector<char> buffer(72 * 1024 * 1024);

    for (size_t i = 0; i < buffer.size(); i += 64) {
        buffer[i] = static_cast<char>(i);
    }

    // Compiler barrier: don’t let optimizer reorder across the flush
    asm volatile("" ::: "memory");
}
...
vector<vector<int>> testing( int numberOfParcelsWanted, const string& output_file) {
    ...
    //write the parameters used to output file
...
vector<vector<int>> question_one(const vector<vector<int>>& parcels) {
    ...
    //open output file
    string output_file = "q1_stats.txt";
    std::ofstream fout(output_file, std::ios::app); // open once in append mode
    if (!fout.is_open()) {
        std::cerr << "Failed to open file\n";
        return {};
    }

    //for loop that doubles number of parcels each time and calls the testing function, upper limit is 10^8
    for (double i = 1; i <= 1.5*100000000; i *= 1.25 ) {
...
vector<int> q2_testing(int numNodes, int numQueries, const string& output_file) {
...
vector<int> question_two(
    ...
    //std::cout << "\n----Q2 started----\n" << std::endl;
     ...
     //open output file
    string output_file = "q2_stats.txt";
    std::ofstream fout(output_file, std::ios::app); // open once in append mode
    if (!fout.is_open()) {
        std::cerr << "Failed to open file\n";
        return {};
...
//function to test the solution
long long q3_testing(int numCities, int numRoads, int numMetro, int maxWeight, bool ensureConnected, const string& output_file) {
     //open output file
...
long long question_three(
    ...
    for (long long i = numCities; i <= 1500000; i = static_cast<long long>(i * 1.25)) {
        ...
        int numRoads = static_cast<int>(std::min(myRoads, 100000LL));
